...
function myErrorHandler(oEvent){
    if(oEvent.bFieldError){
        if(oEvent.oError.sTable === "customer"){
...
function myInitForm(){
    vdf.core.onError.addListener(myErrorHandler);
}
    @code
...
/*
Displays the global error using the vdf.gui.alert method (if available) else 
the window.alert method is used.
*/
display : function(){
