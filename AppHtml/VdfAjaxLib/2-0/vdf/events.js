...
Type:
    Library (object)
Contains:
    vdf.events.DOMHandler   (Prototype)
    vdf.events.DOMEvent     (Prototype)    
    vdf.events.JSHandler    (Prototype)
    vdf.events.JSEvent      (Prototype)
Revisions:
...
@code
function initialize(){
     vdf.events.addDomListener("click", document.getElementById("myButton"), myButton_click, this);
}
...
the vdf.events.DOMHandler object. The name of the event is according to the W3C
...
unloaded). The _oDOMHandlers property is added to the element that contains 
...
Fires the event by creating a vdf.events.DOMEvent object and calling the 
...
/*
Wrapper for the event object that provides a default interface for all the 
different browsers to fetch event properties and influence the handling of the 
event.
*/
...
/*
The handler for an event thrown by the library that contains the references to 
the listeners.. Instead of the DOMHandler this handler is created when the 
prototype that throws it is instantiated instead of when the first listener is 
attached. It represents the actual event.
*/
...
Represents the event object that is given to the listener when the event 
occurs. Has several properties that can be used by handler (like a reference to 
the source). 
*/
