...
/*
If true a load error is already thrown.
...
/*
Loads a library by determining the correct path and generating the script tag
using document.write or document.createElement. It checks if the library isn't
being loaded already before creating the script element.

Internet Explorer only loads before body.onload if script tag is created using
document.write, but this method causes problems if libraries are included later
on from within other files. To prevent these problems we check using
document.readyState if we should use document.write or document.createElement
...
aIncludeJSQueue : [],
/*
Amount of scripts currently loading.

@private
*/
iIncludeJSRunning : 0,
/*
Maximum amount of scripts loading concurrently.
...
/*
Includes a javascript file from the specified location. The amount of 
concurrent includes is limited so the include might be queued.

@param  sUrl    Url of the javascript file to include.
*/
...
/*
Determines of there are (still) javascript files to include. If so it shifts
one url from the queue and includes it. The onload / onreadystatechange events
are used to include to lower the counter and include the next.
...
/*
Includes the cascading style shee with the given name. It searches for the
Library.css include to determine the include path and checks if the include
hasn't been done before.

@param  sName   Name of the style sheet file (without extension).
*/
...
/*
Counter for making duplicate names unique.
...
@param  oControl    Reference to a vdf control.
@param  sDefault    Default name for the control (example: "form", "field" etc).
