/*
 * Visual DataFlex AJAX Library 2.1
...
vdf.gui.TreeView = function TreeView(eElement, oParentControl){
    ...
    this.sName = vdf.determineName(this, "treeview");
...
vdf.definePrototype("vdf.gui.TreeView", {
...
insertNode : function(tNode){
    ...
    if(this.getNodeById(tNode.sId)){
        throw new vdf.errors.Error(1, "Node ID's must be unique within tree", this, [ tNode.sId ]);
    }
...
scanHTMLTree : function(eElement, sParentId){
    ...
    if(eElement.childNodes){
        for(iChild = 0; iChild < eElement.childNodes.length; iChild++){
            if(eElement.childNodes[iChild].nodeType !== 3 && eElement.childNodes[iChild].nodeType !== 8){
                this.scanHTMLTree(eElement.childNodes[iChild], sParentId);
            }
        }
    }
},
...
scrollToElement : function(eElement){
    ...
    iLeft = oElem.left - oDiv.left;
    
    if(iLeft < this.eElement.scrollLeft){
        this.eElement.scrollLeft = iLeft;
    }else if(iLeft + eElement.clientWidth > this.eElement.scrollLeft + this.eElement.clientWidth){
        this.eElement.scrollLeft = iLeft + eElement.clientWidth - this.eElement.clientWidth;
    }
},
...
getCaretPosition : function(eField) {
    ...
    try{
        ...
        vdf.log("getCaretPosition failed: (type: " + eField.type + ", tag: " + eField.tagName + ", name" +eField.name + ")");
    }
...
getSelectionLength : function(eField){
    ...
    try{
        ...
        vdf.log("getSelectionLength failed: (type: " + eField.type + ", tag: " + eField.tagName + ", name" +eField.name + ")");
    }
...
setCaretPosition : function(eField, iCaretPos){
    try{
        ...
        vdf.log("setCaretPosition failed: (type: " + eField.type + ", tag: " + eField.tagName + ", name" +eField.name + ")");
    }
},
...
fadeOpacity : function(eElement, iGoal, iMillis, fFinished, oEnvironment){
    ...
    var oOpacity = {
        ...
        modify : function(){
            ...
            if( parseInt(this.iCurrent, 10) ==  parseInt(this.iGoal, 10)){
                this.stop();
            }
        },
...
crawlDDS : function(fWorker){
    ...
    //  Determine order
    aOrder = this.determineCrawlOrder({}, false, false, false, false, false, false);
    
    //  Call worker for DDs
    for(iCur = 0; iCur < aOrder.length; iCur++){
        oCur = aOrder[iCur];
        
        fWorker.call(oCur.oDD, oCur.oDD, oCur.bConstrainedTo, oCur.bConstrainedFrom, oCur.bServer, oCur.bChild, this === oCur.oDD);
    }
},


determineCrawlOrder : function(oDone, bUp, bDown, bConstrainedTo, bConstrainedFrom, bServer, bChild){
    ...
    if(!oDone[this.sName]){
        ...
        for(iCurrent = 0; iCurrent < this.aConstrainedFrom.length; iCurrent++){
            aResult = aResult.concat(this.aConstrainedFrom[iCurrent].determineCrawlOrder(oDone, bUp, (!bUp), bConstrainedTo, (!bUp && (!bDown || bConstrainedFrom) && !bConstrainedTo), false, (!bServer)));
        }
        
        //  Move to childs
        for(iCurrent = 0; iCurrent < this.aChildren.length; iCurrent++){
            aResult = aResult.concat(this.aChildren[iCurrent].determineCrawlOrder(oDone, bUp, (!bUp), bConstrainedTo, bConstrainedFrom, false, (!bServer)));
        }
        
        
        //  First do constrained to DD where we are constrained to
        if(this.oConstrainedTo){
            aResult = this.oConstrainedTo.determineCrawlOrder(oDone, bDown, (!bDown), (bConstrainedTo || (!bChild && !bConstrainedFrom && !bServer)), bConstrainedFrom, (!bChild), bChild).concat(aResult);
        }
        ...
        for(iCurrent = 0; iCurrent < this.aServers.length; iCurrent++){
            aResult = aResult.concat(this.aServers[iCurrent].determineCrawlOrder(oDone, bDown, (!bDown), bConstrainedTo, bConstrainedFrom, (!bChild), bChild));
        }       
...
generateExtSnapshot : function(bAddFields){
    ...
    this.crawlDDS(function(oDD, bConstrainedTo, bConstrainedFrom, bServer, bChild, bSelf){
        var sBuf, tDD;
...
createExtSnapshotDD : function(bUpdate, bMarkLight, bAddFields, bClone){
    ...
    if(bUpdate){
        this.onUpdate.fire(this, { sAction : null, oAction : null });
...
initialFill : function(){
    ...
    if(this.oForm.onBeforeInitialFill.fire(this, {})){
        ...
        this.crawlDDS(function(oDD, bConstrainedTo, bConstrainedFrom, bServer, bChild, bSelf){
            ...
            oDD.onUpdate.fire(oDD, { sAction : "initialFill", oAction : oAction } );
...
init : function(){
    if(!this.oForm || this.oForm.sControlType !== "vdf.core.Form"){
        throw new vdf.errors.Error(134, "Reference to vdf.core.Form required", this, [ this.sName ]);
    }
},
...
vdf.definePrototype("vdf.core.FormBase", {
