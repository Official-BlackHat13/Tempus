...
/*
The lookup can be used let the user select a record from a list with multiple 
columns. The lookup shares it engine (the super class vdf.core.List) with the 
grid. The lookup only works inside a form and will bind itself to its server DD 
using the vdfServerTable property. The vdfMainTable property defines on which 
table the finds should be performed. Within the lookup fields from the main 
table and its server tables can be used. 

Within the HTML the list is defined using a table element having one or two 
rows. The important row is the display row which is defined by adding 
vdfRowType="display" to the row (tr) element. This row can contain Data Entry 
Objects (usually of the vdf.deo.DOM type) that will be filled with values from 
the record. The display row will be duplicated for each record displayed in the 
list.

@code
<table vdfControlType="lookup" vdfControlName="customer_lookup" vdfMainTable="customer" vdfRowLength="10" vdfFocus="true" vdfFixedColumnWidth="true" vdfDetermineWidth="true">
