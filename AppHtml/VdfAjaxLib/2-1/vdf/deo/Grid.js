...
/*
This class contains the implementation of the Grid component. It extends the 
vdf.core.List with the functionality to editor the selected row. The Grid 
implements the Data Entry Object interface (defined in vdf.core.DEO) so the 
vdf.core.ClientDataDictionary will is able to send it refresh messages. 
Important in this is the sServerTable property which defines to which DDO the 
grid will register itself. The Grid should be used within a vdf.core.Form 
component to be able to work properly.

Within the HTML the Grid can be declared using a table which should have at 
least a display row and an edit row. The display row is duplicated for all the 
records that are displayed by the grid. Data Entry Objects within the row 
(usually DOM fields) should be used to display the values of the record based on 
their data binding. The edit row represents the selected row of the grid and 
should also contain Data Entry Objects to display the record values and to allow 
the user to edit them. The DEO's used in the edit row are registered at the form 
like regular fields and will behave like regular fields. The vdfRowType 
attribute should be used on the row (tr) element so the grid engine knows which 
row is being defined. For the display and edit row these are 
vdfRowType="display" and vdfRowType="edit".

An extra row that can be defined is the header row using vdfRowType="header". 
The header row type also can contain Data Entry Objects (usually DOM fields). If 
the property bAutoLabel is set to true these fields will be filled with the 
short label of the database field to which it is bound.

@code
<table vdfControlType="grid" vdfControlName="numbers_grid" vdfMainTable="numbers" vdfServerTable="numbers" vdfAlwaysInOrder="false" vdfRowLength="8">
    <tr vdfRowType="header">
        <th vdfDataBinding="numbers__phone_number">Phone Number</th>
        <th vdfDataBinding="numbers__description">Description</th>
    </tr>
...
@code
The example above shows a simple grid displaying two fields from the numbers 
table. Important properties here are the vdfMainTable and the vdfServerTable. 
The main table is the table on which the finds are actually performed. The 
vdfAlwaysInOrder is set to false because else wise the grid would expect new 
records always to stay on the bottom (which is the case with hidden auto number 
